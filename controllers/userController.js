...
module.exports = {
    ...
    confirmation: async (req, res) => {
        ...
        if (token) {
            if (moment(token.token_created_at).add(1, 'd').toDate() >= Date.now()) {
                ...
                if (user) {
                    if (user.confirmed) {
                        console.log({ type: 'already-verified', msg: 'Este usuario fue verificado.' });
                    } else {
                    ...
                    res.status(201).json({ msg: 'No pudimos encontrar al usuario con este token.' });
                }
            } else {
                res.status(400).json({ type: 'not-verified', msg: 'No pudimos encontrar un token válido. Su token expiro.' });
            }
            return helper.generateAccessData(token, res);
        } else {
            res.status(400).json({ type: 'not-verified', msg: 'No pudimos encontrar un token válido.' });
        }
    },
    ...
    resendToken: async (req, res) => {
        ...
        try {
            const user = await models.user.findOne({ where: { email: req.body.email } });
            if (user.confirmed) {
                return res.status(422).json({ message: "Esta cuenta ya fue verificada" });
            } else {
                const token = crypto.randomBytes(16).toString('hex')
        ...
        if (token) {
            if (moment(token.token_password_created_at).add(30, 'm').toDate() >= Date.now()) {
                if (req.body.new_password === req.body.verify_password) {
                    ...
                    if (newUser) {
                        console.log("Se cambio el password. Por favor inicie sesion.");
                        return helper.generateAccessData(newUser, res);
                    }
                    return helper.generateAccessData(token, res);
                } else {
                    res.status(400).json({ msg: 'Los password no coinciden' })
                }
            } else {
                res.status(400).json({ type: 'not-verified', msg: 'No pudimos encontrar un token válido. Su token expiro.' });
            }
    ...
    updateUser: async (req, res) => {
        ...
        try {
            ...
            if (user) {
                ...
                return res.status(400).send({ status: 400, message: "No existe el usuario" })
            }
        } catch (error) {
    ...
    listCountry: (req, res) => {
        models.country.findAll().then(countries => {
            if (countries) {
                ...
                return res.status(201).json({ status: 201, msg: "No hay paises registrados" })
            }
        })
    }
}